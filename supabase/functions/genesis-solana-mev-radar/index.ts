...
async function runRadar() {
  ...
  for (let attempt = 0; attempt < Math.min(MAX_TOKEN_ATTEMPTS, tokenIndexes.length); attempt++) { const token = TOKENS[tokenIndexes[attempt]]; if (!token) continue; const result = await probeToken(token, notionalUsd, evidence); probeErrors.push(...result.errors.map((e) => `${token.symbol}:${e}`)); if (result.candidates.length) { probe = result; fallbackUsed = attempt > 0; break; } if (isNoDirectRoute(result.errors)) await persistRouteUnavailable(db, token, notionalUsd, selectionMode, ranking); }
