...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!['ALLOW_NEW_HYPOTHESIS','ALLOW_RESEARCH_CHALLENGE','ALLOW_FORWARD_CONFIRMATION'].includes(String(proposal.decision))) return json({ ok: true, engineVersion: ENGINE_VERSION, verdict: 'DATA_BLOCKED', reason: `Hypothesis gate decision ${proposal.decision} does not authorize research.`, proposalId, hypothesisFingerprint: proposal.hypothesis_fingerprint, executionAuthority: false, capitalEligible: false, liveOrders: false });
    const spec = proposal.canonical_spec || {}, family = String(proposal.family || 'UNKNOWN'), dataClass = String(spec.dataClass || '').toLowerCase();
    if (!dataClass.includes('binance_usdm')) return json({ ok: true, engineVersion: ENGINE_VERSION, verdict: 'DATA_BLOCKED', reason: 'efg_v1.1 currently supports trusted Binance USD-M futures evidence only.', proposalId, hypothesisFingerprint: proposal.hypothesis_fingerprint, family, executionAuthority: false, capitalEligible: false, liveOrders: false });
    const stateRows = await rest(`org_state?key=eq.${FUTURES_STATE_KEY}&select=value,updated_at&limit=1`), stateRow = stateRows?.[0], state: any = parseJson(stateRow?.value, null);
    if (!state?.ok || state?.spotReferenceUsed !== false || !state?.evidence) return json({ ok: true, engineVersion: ENGINE_VERSION, verdict: 'DATA_BLOCKED', reason: 'Trusted futures-native market evidence is missing or invalid.', proposalId, hypothesisFingerprint: proposal.hypothesis_fingerprint, family, executionAuthority: false, capitalEligible: false, liveOrders: false });
    const costModel = proposal.cost_model || spec?.costModel || {}, modeledCostBps = Number(costModel.roundTripBps ?? 0);
    const assessment = family === 'CRYPTO_CROSS_ASSET_FUNDING_SPREAD' ? fundingSpreadAssessment(spec, state, modeledCostBps) : basisAssessment(spec, state, modeledCostBps);
    const sourceHash = await sha256(JSON.stringify((assessment.rows || []).map((r: any) => ({ symbol: r.symbol, sourceEvidenceHash: r.sourceEvidenceHash }))));
    const insert = { proposal_id: proposal.proposal_id, hypothesis_fingerprint: proposal.hypothesis_fingerprint, family, engine_version: ENGINE_VERSION, mode: assessment.mode, source_state_key: FUTURES_STATE_KEY, source_evidence_hash: sourceHash, observed_at: state.completedAt ?? stateRow?.updated_at ?? new Date().toISOString(), gross_edge_bps: assessment.selected?.grossEdgeBps ?? assessment.selected?.grossFundingBps ?? null, modeled_cost_bps: modeledCostBps, safety_buffer_bps: assessment.safetyBufferBps, required_gross_bps: assessment.requiredGrossBps, edge_cost_ratio: assessment.selected?.edgeCostRatio ?? null, margin_bps: (assessment.selected?.grossEdgeBps ?? assessment.selected?.grossFundingBps) == null ? null : Number(assessment.selected?.grossEdgeBps ?? assessment.selected?.grossFundingBps) - Number(assessment.requiredGrossBps), verdict: assessment.verdict, reason: assessment.reason, details: { ...assessment.details, selectedOpportunity: assessment.selected, proposalDecision: proposal.decision, dataClass, policy: 'economic prefilter only; never grants execution authority' }, execution_authority: false, capital_eligible: false, live_orders: false };
    ...
    return json({ ok: true, engineVersion: ENGINE_VERSION, checkId: created?.[0]?.check_id ?? null, createdAt: created?.[0]?.created_at ?? null, proposalId, hypothesisFingerprint: proposal.hypothesis_fingerprint, family, verdict: assessment.verdict, reason: assessment.reason, modeledCostBps: round(modeledCostBps), safetyBufferBps: round(assessment.safetyBufferBps), requiredGrossBps: round(assessment.requiredGrossBps), selectedOpportunity: assessment.selected, assets: assessment.rows, executionAuthority: false, capitalEligible: false, liveOrders: false });
