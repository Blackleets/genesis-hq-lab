  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#0a0e1a', border: '1px solid #1e2a3a', borderRadius: 8, overflow: 'hidden' }}>
      <div style={{ padding: '8px 12px', borderBottom: '1px solid #1e2a3a', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <span style={{ fontSize: 10, color: '#4b5563', fontWeight: 600, letterSpacing: 1 }}>LIVE TOKEN FEED</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {tokens.length === 0 ? (
          ...
          <div key={t.mint} style={{
            ...
            alignItems: 'center',
            gap: 8,
            background: flash === t.mint ? 'rgba(34,197,94,0.08)' : 'transparent',
            transition: 'background 0.4s',
          }}>
            <div style={{ minWidth: 44, fontWeight: 700, fontSize: 11, color: '#e2e8f0' }}>
              {t.symbol}
            </div>
            <div style={{ flex: 1, fontSize: 10, color: '#6b7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {t.name}
            </div>
