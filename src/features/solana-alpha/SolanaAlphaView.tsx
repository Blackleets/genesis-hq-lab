    ...
    <div className="solana-alpha-root" style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto', overflowX: 'hidden', background: '#050810', padding: 10, gap: 8, position: 'relative' }}>
      ...
      <div className="solana-alpha-header">
        ...
        <div className="solana-alpha-actions">
          ...
          {loading && !error && (
            <div style={{ fontSize: 9, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 4, padding: '4px 8px' }}>
              ⟳ conectando...
            </div>
          )}
          {error && (
            <div style={{ fontSize: 9, color: loading ? '#ffb547' : '#ff4757', background: loading ? 'rgba(255,181,71,0.1)' : 'rgba(255,71,87,0.1)', borderRadius: 4, padding: '4px 8px', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {error}
            </div>
          )}
        </div>
