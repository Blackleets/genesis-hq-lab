...
# Same fixed universe as cross-sectional momentum; no universe mining.
UNIVERSE = {
    "SPY": "US_large_cap",
    "QQQ": "US_growth_tech",
    "IWM": "US_small_cap",
...
OUT = Path(__file__).resolve().parent / "short-term-reversal-output"
...
def build_portfolio(prices: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame, pd.DataFrame]:
    ...
    recent_return = prices / prices.shift(REVERSAL_LOOKBACK) - 1.0
    # Higher score = worse recent return = candidate long; lower = recent winner = candidate short.
    ...
    first_eligible = max(REVERSAL_LOOKBACK, VOL_LOOKBACK + 1)
    rebalances = 0
    signal_gaps = []
    ...
    for i in range(first_eligible, len(prices)):
        if (i - first_eligible) % REBALANCE_DAYS != 0:
            continue
        signal = reversal_score.iloc[i].replace([np.inf, -np.inf], np.nan).dropna()
        ...
        shorts = ranked.index[:n]   # recent winners
        longs = ranked.index[-n:]   # recent losers

        long_inv = 1.0 / vol.loc[longs]
        short_inv = 1.0 / vol.loc[shorts]
        row = pd.Series(0.0, index=prices.columns)
        row.loc[longs] = 0.5 * long_inv / long_inv.sum()
        row.loc[shorts] = -0.5 * short_inv / short_inv.sum()
        weights.iloc[i] = row
        rebalances += 1
        signal_gaps.append(float(signal.loc[longs].mean() - signal.loc[shorts].mean()))

    weights = weights.ffill().fillna(0.0)
    active = weights.abs().sum(axis=1) > 0
    if not active.any():
        raise RuntimeError("no active weights created")
    ...
    diagnostics.attrs["avg_reversal_signal_gap"] = float(np.mean(signal_gaps)) if signal_gaps else None
...
def print_result(label: str, result: dict) -> None:
    ...
    print(f"SLICE_RESULT name={label} n={s['n_trades']} WR={s['win_rate']*100:.1f}% "
          f"PF={s['profit_factor']:.4f} EV={s['expectancy_pct']:.4f}% tstat={s['tstat']:.4f} "
          f"maxDD={s['max_dd_pct']:.2f}% h1={result['h1_mean_pct']} h2={result['h2_mean_pct']} "
          f"consistent={result['consistent']} MC_p5={mc.get('p5_total_return_pct')} "
          f"verdict={s['verdict']} gate_pass={gate_pass(result)}")


def main() -> None:
    print("=== GENESIS CROSS-SECTIONAL SHORT-TERM REVERSAL V1 ===")
    print(f"DATA_POLICY yfinance_adjusted_real_only start={START} end_exclusive={END}")
    print(f"UNIVERSE_POLICY fixed_ex_ante_assets={len(UNIVERSE)} same_as_cross_sectional_momentum=true")
    print(f"SIGNAL_POLICY loser_minus_winner lookback={REVERSAL_LOOKBACK} rebalance={REBALANCE_DAYS} top_fraction={TOP_FRACTION}")
    print(f"WEIGHT_POLICY long_gross=0.5 short_gross=0.5 inverse_vol_{VOL_LOOKBACK}d_causal=true")
    ...
    print(f"PORTFOLIO_DIAGNOSTICS rebalances={diag.attrs['rebalances']} avg_reversal_signal_gap={diag.attrs['avg_reversal_signal_gap']} "
    ...
    report = {
        "engine": "short_term_reversal_v1",
        "data_policy": "yfinance_adjusted_real_only",
        "universe": UNIVERSE,
        "params": {"lookback": REVERSAL_LOOKBACK, "rebalance_days": REBALANCE_DAYS, "vol_lookback": VOL_LOOKBACK,
                   "top_fraction": TOP_FRACTION, "cost_bps": COST_BPS},
        "gates": GATES,
    ...
    path = OUT / "short_term_reversal_v1_report.json"
