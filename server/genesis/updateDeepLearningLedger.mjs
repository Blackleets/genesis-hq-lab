...
async function main(){
  ...
  const ledger=await readJson(ledgerPath,{version:'edge_deep_learning_ledger_v3_structural_gate',paperOnly:true,liveOrders:false,executionAuthority:false,capitalEligible:false,killed:{}});
  ledger.version='edge_deep_learning_ledger_v3_structural_gate';
  ...
  console.log(JSON.stringify({ok:true,version:ledger.version,controlsEvaluated:controls.length,newlyKilled,killed:Object.keys(ledger.killed).length}));
}
