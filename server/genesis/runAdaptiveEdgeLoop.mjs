...
async function main(){
  ...
  const results=[];
  for(const item of queue){const d=datasets.get(`${item.market.pair}:${item.market.tf}`),tr=simulate(d.rows,d.prep,d.market,item.spec),train=metrics(tr.filter(x=>x.relativeIndex<TRAIN)),validation=metrics(tr.filter(x=>x.relativeIndex>=TRAIN&&x.relativeIndex<VALID)),wf=walk(tr),pass=researchGate(train,validation,wf),score=selectionScore(train,validation,wf);results.push({id:item.id,parentVariant:item.parentVariant,priority:item.priority,market:item.market,spec:item.spec,researchStatus:pass?'PROMOTABLE_TO_ONE_SHOT_AUDIT':'REJECT',selectionScore:score,train,validation,walkForward:wf});}
  results.sort((a,b)=>b.selectionScore-a.selectionScore);
  const promotable=results.filter(x=>x.researchStatus==='PROMOTABLE_TO_ONE_SHOT_AUDIT');
  const snapshot={ok:true,version:'adaptive_edge_loop_v1',mode:'RESEARCH_ONLY',paperOnly:true,liveOrders:false,executionAuthority:false,capitalEligible:false,completedAt:new Date().toISOString(),methodology:{selectionUsesHoldout:false,selectionInputs:['train','validation','walkForward'],holdoutPolicy:'ONE_SHOT_AUDIT_ONLY',barsPerMarket:MAX_BARS},sourceQueueCompletedAt:q.completedAt??null,tested:results.length,promotableToAudit:promotable.length,verdict:promotable.length?'AUDIT_CANDIDATE_FOUND':'NO_ADAPTIVE_EDGE_FOUND',best:results[0]??null,auditQueue:promotable.slice(0,3).map(x=>({id:x.id,market:x.market,spec:x.spec,parentVariant:x.parentVariant,selectionScore:x.selectionScore,reason:'Passed train+validation+walk-forward without consulting holdout.'})),top:results.slice(0,10)};
  ...
  console.log(JSON.stringify({ok:true,tested:snapshot.tested,promotableToAudit:snapshot.promotableToAudit,verdict:snapshot.verdict,best:snapshot.best?.id??null}));
}
