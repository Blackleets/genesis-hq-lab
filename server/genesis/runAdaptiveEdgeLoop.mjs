...
async function writeSnapshot(out,history,snapshot){await mkdir(dirname(out),{recursive:true});await writeFile(out,JSON.stringify(snapshot,null,2)+'\n');await appendFile(history,JSON.stringify(snapshot)+'\n');}
...
async function main(){
  ...
  const all=q.queue??[],high=all.filter(x=>['P0','P1'].includes(x.priority)),queue=(high.length?high.slice(0,24):all.filter(x=>x.priority==='P2').slice(0,10)),queueMode=high.length?'HIGH_PRIORITY':'P2_DEEPENING';
  if(!queue.length){
    const snapshot={ok:true,version:'adaptive_edge_loop_v5_empty_handoff',mode:'RESEARCH_ONLY',paperOnly:true,liveOrders:false,executionAuthority:false,capitalEligible:false,completedAt:new Date().toISOString(),methodology:{selectionUsesHoldout:false,selectionInputs:['train','validation','walkForward'],holdoutPolicy:'ONE_SHOT_AUDIT_ONLY',parentControls:true,multiControlDeepLearning:true,p2Fallback:true,emptyQueueHandoff:true,barsPerMarket:MAX_BARS},queueMode:'EMPTY_QUEUE_DISCOVERY_HANDOFF',sourceQueueCompletedAt:q.completedAt??null,tested:0,controls:0,promotableToAudit:0,verdict:'NO_QUEUE_DISCOVERY_REQUIRED',bestMutation:null,control:null,controlSet:[],auditQueue:[],top:[]};
    await writeSnapshot(out,history,snapshot);console.log(JSON.stringify({ok:true,queueMode:snapshot.queueMode,tested:0,verdict:snapshot.verdict}));return;
  }
  ...
  results.sort((a,b)=>b.selectionScore-a.selectionScore);const promotable=results.filter(x=>x.role==='MUTATION'&&x.researchStatus==='PROMOTABLE_TO_ONE_SHOT_AUDIT'),controls=results.filter(x=>x.role==='CONTROL');
  const snapshot={ok:true,version:'adaptive_edge_loop_v5_empty_handoff',mode:'RESEARCH_ONLY',paperOnly:true,liveOrders:false,executionAuthority:false,capitalEligible:false,completedAt:new Date().toISOString(),methodology:{selectionUsesHoldout:false,selectionInputs:['train','validation','walkForward'],holdoutPolicy:'ONE_SHOT_AUDIT_ONLY',parentControls:true,multiControlDeepLearning:true,p2Fallback:true,emptyQueueHandoff:true,barsPerMarket:MAX_BARS},queueMode,sourceQueueCompletedAt:q.completedAt??null,tested:results.length,controls:controls.length,promotableToAudit:promotable.length,verdict:promotable.length?'AUDIT_CANDIDATE_FOUND':'NO_ADAPTIVE_EDGE_FOUND',bestMutation:results.find(x=>x.role==='MUTATION')??null,control:controls[0]??null,controlSet:controls,auditQueue:promotable.slice(0,12).map(x=>({id:x.id,hypothesisKey:x.hypothesisKey,market:x.market,spec:x.spec,parentVariant:x.parentVariant,selectionScore:x.selectionScore,reason:'Passed train+validation+walk-forward without consulting holdout.'})),top:results.slice(0,12)};
  await writeSnapshot(out,history,snapshot);console.log(JSON.stringify({ok:true,queueMode,tested:snapshot.tested,controls:snapshot.controls,promotableToAudit:snapshot.promotableToAudit,verdict:snapshot.verdict,best:snapshot.bestMutation?.id??null}));
}
