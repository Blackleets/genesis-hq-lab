// Persist one RESEARCH_ONLY OKX RPI order-book snapshot as durable evidence.
...
export async function captureRpiOrderBook({ instId = 'BTC-USDT-SWAP', depth = 20, out } = {}) {
...
if (process.argv[1] && process.argv[1].endsWith('rpiOrderBookCapture.mjs')) {
  ...
  captureRpiOrderBook({ instId: valueAfter('--inst') || 'BTC-USDT-SWAP', depth: valueAfter('--depth') || 20, out: valueAfter('--out') })
