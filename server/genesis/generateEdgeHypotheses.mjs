...
async function main(){
  ...
  const eligible=sources.filter(x=>!durableKilled.has(x.hypothesisKey));
  ...
  for(const source of ranked.slice(0,6)){
    ...
    queue.push({id:`${source.variantKey??source.hypothesisKey}:control`,role:'CONTROL',priority,parentVariant:source.variantKey??source.hypothesisKey,hypothesisKey:source.hypothesisKey,market:source.market,spec:source.candidate,rationale:'Control replay of the parent variant. It is not a new hypothesis and prevents us from confusing data drift with parameter improvement.',sentinelCritique:critique,expectedFalsification:'Control must remain directionally consistent on train and validation; otherwise pause mutations and diagnose regime/data drift.',sourceEvidence:{train:source.train,validation:source.validation,holdout:source.holdout,walkForward:source.walkForward}});
  ...
  const snapshot={ok:true,version:'edge_hypothesis_generator_v4_durable_learning',mode:'RESEARCH_ONLY',paperOnly:true,liveOrders:false,executionAuthority:false,capitalEligible:false,completedAt:new Date().toISOString(),methodology:{selectionUsesHoldout:false,balancedNeighborhood:true,parentControl:true,deepValidationOverridesShortWindow:true,durableDeepMemory:true},sourceVerdict:edge.verdict,queueSize:queue.length,deepOverrides,focus:ranked[0]?{hypothesisKey:ranked[0].hypothesisKey,variantKey:ranked[0].variantKey??null,market:ranked[0].market,candidate:ranked[0].candidate,status:ranked[0].status,researchScore:ranked[0].researchScore,sentinelCritique:sentinelCritique(ranked[0])}:null,queue:queue.slice(0,30)};
  ...
  console.log(JSON.stringify({ok:true,queueSize:snapshot.queueSize,focus:snapshot.focus?.hypothesisKey??null,deepKilled:deepOverrides.length,selectionUsesHoldout:false}));
}
