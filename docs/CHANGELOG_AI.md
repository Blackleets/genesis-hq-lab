...
## 2026-09-06 — ASTRA / Codex
- Branch: feat/founder-real-control-room (base feat/genesis-life-os at c13138e).
- Summary: Added read-only Founder Live Control, deterministic signed-evidence readiness, Hermes registry and seven agent roles/desks. Replaced static online/P&L fallback claims and no-op buttons with backend states. No live flags or order paths changed.
- Security: Migrated wallet bearer JSON/sessionStorage to Secure HttpOnly cookies with same-origin write protection. Retired old browser connector credential forms and direct external dispatch because secrets and privileged connector requests belong on the backend. Purged legacy connector config during hydration. No files deleted.
- UI reliability: Fixed global health hook accepting HTTP 503/error envelopes and crashing the header. Added mobile workspace selection; removed the static office-online header label.
- Files touched: api/genesis/founder.js; api/auth/{verify,session,logout}.js and tests; api/_lib/{sessionAuth,sessionCookie}.js; server/index.mjs; server/genesis/{founderReadiness,founderHttp,hermesConnectorRegistry}.mjs and tests; src/components/crypto/{GenesisCommandCenter,FounderControlRoom}.tsx; src/services/{founderClient,connectorClient}.ts; src/core/auth/*; src/core/store/{genesisStore,persistence}.ts; src/core/data/moduleRegistry.ts; src/hooks/useTruthLayer.ts; src/App.tsx; src/ui/{GenesisHeader,IntegrationsView}.tsx; src/workflows/{BotCreatorView,QuantBotView}.tsx; scripts/verifyFounderUi.mjs; .github/workflows/founder-control-checks.yml; docs/FOUNDER_CONTROL_*.md and evidence screenshots.
- Verification: npm run build — OK (TypeScript included); 57 readiness/HTTP/ledger tests + 19 auth tests pass; browser desktop/mobile, failed backend, expired readiness and zero-write review pass. Targeted new-module lint passes. Existing Fast Refresh export lint issue remains. Production accounts, profitability, deployed auth and live cutover are not verified. The new roles other than the request-time SENTINEL evaluator are not represented as running.
