...
- Gate repair: the existing Founder Vercel contract test now awaits its asynchronous handler and supplies the minimal request URL/headers contract; production Founder behavior is unchanged.
- Auth gate repair: the session response test now compares millisecond timestamps with the 24-hour TTL in milliseconds; JWT expiry remains separately asserted in seconds and production auth behavior is unchanged.
